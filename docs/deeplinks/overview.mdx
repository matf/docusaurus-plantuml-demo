...
digraph {
  ...
  runbook [
    label="Retry loop →\n(other page)",
    URL="/docusaurus-plantuml-demo/docs/deeplinks/runbook#graph?highlight-node=RETRY_LOOP_9",
    fontcolor="#4a6fa5",
    color="#4a6fa5"
  ];
...
The **Retry loop →** node in the Graphviz diagram above carries the first of those as its
`URL`, so the diagram itself links into the other page's diagram.
...
| 3   | Self-anchor       | Graphviz | `node [URL="#graph?highlight-node=…"]`          |
...
- **Links inside a Graphviz diagram are ordinary links.** One that points at another page
  leaves the single-page app and loads that page fresh. Same-page ones (`#graph?…`) never do.
- **PlantUML `[[url]]` links are not clickable** — the bundled engine draws the link text but
  emits no anchor. That is why PlantUML deep-link ids ride on aliases rather than on links.
