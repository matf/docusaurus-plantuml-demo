...
[the deep links page](./overview.mdx) and from the nodes on
[Links on nodes](./from-nodes.mdx) — and it links back both ways, from prose and from inside
its own diagrams.
...
the picture. **Order service** is blue because it carries a `[[…]]` link of its own, back to
the overview page's Graphviz diagram.

```plantuml title="Recovery path"
@startuml
skinparam componentStyle rectangle

component "Order service" as ORDER_SERVICE_42 [[/docs/deeplinks/overview#graph?highlight-node=ORDER-DB-1]]
...
That node being both a link and a link target is the whole idea in one box: press it to go
somewhere, or address it to arrive here.
...
digraph {
  ...
  // Leftmost on purpose: the figure's toolbar sits over the top-right corner, and a link
  // nobody can press is a poor demonstration of a link.
  back [
    label="← Orders DB\n(overview page)",
    URL="/docs/deeplinks/overview#graph?highlight-node=ORDER-DB-1",
    fontcolor="#4a6fa5",
    color="#4a6fa5"
  ];
  ...
  back -> dlq [style=dotted, label="procedure"];
  dlq -> drain;
  drain -> replay;
  drain -> discard;
}
...
highlighted, without a page reload. Note the path is written `/docs/deeplinks/overview`: the
site's `baseUrl` is added for you, the same as in a markdown link.
